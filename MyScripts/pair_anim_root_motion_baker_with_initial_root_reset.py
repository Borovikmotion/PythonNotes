...
class ObjectRebaker(object):
    ...
    def resetRootTransform(self):
        tempLoc = cmds.spaceLocator(n="reset_LOC")
        cmds.setAttr("reset_LOC.rotateX", -90)
        tempCon1 = cmds.pointConstraint(tempLoc, self.objectCtrl, mo=0)
        tempCon2 = cmds.orientConstraint(tempLoc, self.objectCtrl, mo=0)
        cmds.bakeResults("root", time = (self.minT, self.maxT))
        cmds.delete(tempLoc)
    ...
    def main(self):
        ...
        for f in self.files:
            ...
            # move root to scene 0 0 0 with 0 0 0 rotations
...
rebake1 = ObjectRebaker(path = "D:/Work/66_pair_animation_root_motion/Source_180_fbx", objName = "root", childObjects = ["pelvis", "ik_hand_gun", "ik_foot_root"], animSourceObject = "ik_foot_root", offsetPosX = 0, offsetPosY = 0, offsetPosZ = 0, offsetRotX = 0, offsetRotY = 0, offsetRotZ = 0, moveChilds = False)
