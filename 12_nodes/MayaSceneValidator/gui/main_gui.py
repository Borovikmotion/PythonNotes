...
from maya.app.general.mayaMixin import MayaQWidgetDockableMixin # an addition to class 
import os
...
class ValidatorGUI(MayaQWidgetDockableMixin, QtWidgets.QDockWidget):
    ...
    def setup_ui(self):
        ...
        #button fix 
        self.button_fix = QtWidgets.QPushButton("Run Checks")
        # self.button_fix.setMaximumWidth(32)
        ...
        self.combo_presets.setMaximumWidth(350)
        self.buttons_layout.addWidget(self.combo_presets)

        for i in self.resources.get_presets():
            preset_name = os.path.split(i)[-1]
            preset_name_no_ext = os.path.splitext(preset_name)[0]
            self.combo_presets.addItem(preset_name_no_ext)
    ...
    def save_config_preset(self, text):
        ...
        log(message="config.ini saved. New preset is {0}".format(text))
    ...
    def fix_validator(self, rule_name=None):
        # print(rule_name)
        if self.scroll_layout:
            if self.scroll_layout.count() > 0:
                for i in range(0, self.scroll_layout.count()):
            ...
            if self.scroll_layout.count() > 0:
                for i in range(0, self.scroll_layout.count()):
                    ...
                    if rule_name==name:
                            ...
                            # log(message="{} check".format(name), category="Fix")
                            if not result:
                                self.clean_scroll(rule_name=name)
                            else:
